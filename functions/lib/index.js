...
exports.onCompanyProfileWrite = exports.onMeetingCreate = exports.onTaskWrite = exports.onNewChatMessage = void 0;
const functions = require("firebase-functions");
const admin = require("firebase-admin");
admin.initializeApp();
// E2EE Decrypt helper for server notifications preview
const decryptText = (encrypted, key) => {
    if (!encrypted)
        return '';
    if (!encrypted.startsWith('[E2EE] '))
        return encrypted;
    try {
        ...
        return xor.split('').map((char, i) => String.fromCharCode(char.charCodeAt(0) ^ key.charCodeAt(i % key.length))).join('');
    }
    catch (e) {
...
exports.onNewChatMessage = functions.firestore
    ...
    const employeesSnapshot = await admin.firestore()
        ...
        .get();
    const recipientUids = [adminUid];
    employeesSnapshot.forEach(doc => {
        const emp = doc.data();
        if (emp.uid)
            recipientUids.push(emp.uid);
    });
 ...
 */
exports.onCompanyProfileWrite = functions.firestore
    ...
    if (afterLeads.length > beforeLeads.length) {
        // New lead registered
        const newLead = afterLeads.find((al) => !beforeLeads.some((bl) => bl.id === al.id));
        if (newLead) {
        ...
        for (const al of afterLeads) {
            const bl = beforeLeads.find((b) => b.id === al.id);
