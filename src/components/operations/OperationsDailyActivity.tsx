...
export function OperationsDailyActivity({ profileRef, readOnly }: OperationsDailyActivityProps) {
  ...
  const [feedback, setFeedback] = useState(profile?.opsDailyFeedback || '');
  const [isSaving, setIsSaving] = useState(false);

  useEffect(() => {
    if (profile?.opsDailyFeedback !== undefined) {
      setFeedback(profile.opsDailyFeedback);
    }
  }, [profile?.opsDailyFeedback]);
  ...
  const handleSaveFeedback = async () => {
    ...
    try {
      await setDocumentNonBlocking(profileRef, { 
        opsDailyFeedback: feedback,
        opsDailyFeedbackSavedAt: nowStr
      }, { merge: true });
    } catch (err) {
      console.error("Error saving ops daily feedback:", err);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const currentWeekTasks = useMemo(() => {
    return allOpsTasks.filter((t: any) => {
  ...
  }, [allOpsTasks, currentWeekIndex, currentMonthIndex]);
  ...
  const recommendedTasks = useMemo(() => {
    ...
    if (activeWeekPlan) {
      const actions = activeWeekPlan.opsActions || [];
      if (actions.length > 0) {
        return actions.map((act: string) => ({
          title: act,
          description: `Strategic operations action item scheduled for ${activeWeekPlan.week || 'this week'}.`,
          priority: 'high',
          category: 'Operations'
        }));
      }
    ...
    <div className="space-y-8 animate-in fade-in duration-300">
            ...
            <CardContent className="p-0 divide-y divide-slate-100">
              {currentWeekTasks.length === 0 ? (
                <div className="p-8 text-center text-muted-foreground flex flex-col items-center justify-center gap-2">
                  ...
                  <p className="text-xs font-semibold">No strategic Operations tasks assigned for Week {currentWeekIndex + 1}.</p>
      ...
      <Card className="border-2 border-indigo-100 bg-white shadow-sm mt-6">
        <CardHeader className="pb-3 border-b">
          ...
          <CardDescription className="text-xs">
            Reflect on how today's Operations activities went. The AI will analyze this feedback during adaptation to optimize your strategy.
          </CardDescription>
        </CardHeader>
        <CardContent className="pt-4 space-y-3">
          <textarea
            ...
            rows={3}
            placeholder="Type your notes or feedback here (e.g. Set up onboarding templates, but SaaS tools subscriptions check is still pending)..."
            value={feedback}
          ...
          <div className="flex items-center justify-between gap-4 pt-1">
            <div className="text-[10px] text-muted-foreground font-semibold flex items-center gap-1.5">
              {profile?.opsDailyFeedbackSavedAt ? (
                <>
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                  <span>Last saved: {profile.opsDailyFeedbackSavedAt}</span>
                </>
              ) : (
                <>
