...
export const DEFAULT_FINANCE_TARGETS: FinanceTargets = {
...
interface FinanceWeeklyDashboardProps {
...
export function FinanceWeeklyDashboard({ profileRef, readOnly }: FinanceWeeklyDashboardProps) {
  ...
  const handleSaveFeedback = async () => {
    ...
    try {
      await setDocumentNonBlocking(profileRef, { 
        financeWeeklyFeedback: feedback,
        financeWeeklyFeedbackSavedAt: nowStr
      }, { merge: true });
    } catch (err) {
      console.error("Error saving finance weekly feedback:", err);
    } finally {
      setIsSaving(false);
    }
  };
        ...
        <CardHeader className="pb-3 border-b bg-indigo-50/5 flex flex-col md:flex-row md:items-center justify-between gap-3">
          <div>
            <CardTitle className="text-base font-black text-slate-900 flex items-center gap-2">
              ...
              Strategic Finance Weekly Actions ({weeklyStats.completed} / {weeklyStats.total})
            </CardTitle>
        ...
        <CardContent className="pt-4 space-y-3">
          ...
          <div className="flex items-center justify-between gap-4 pt-1">
            <div className="text-[10px] text-muted-foreground font-semibold flex items-center gap-1.5">
              {profile?.financeWeeklyFeedbackSavedAt ? (
                <>
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                  <span>Last saved: {profile.financeWeeklyFeedbackSavedAt}</span>
                </>
              ) : (
                <>
